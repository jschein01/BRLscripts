...
def plot_trials(windowSize=75, data=None, plot=True):
    ...
    if saveInput == 'y':
        save_data_portion(timeCol, voltCol, startIndex, endIndex, f'trial-'+medium+'-.csv')
    # Create a plot for each trial
    plt.figure(figsize=(10, 5))
    
    voltCol.plot(x=voltCol, y=timeCol, xlim=(startIndex, endIndex), ylim=(-0.02, maxVal*1.1), title=f'Trial ' + medium +' - Voltage vs Time')
