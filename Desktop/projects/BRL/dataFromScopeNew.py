...
def sync_data(dir, savePath, threshold=threshold):
    print(f"Syncing data in directory: {dir}")
    index = 0
...
def main():
    ...
    sync_data(dataPath, savePath=savePath, threshold=threshold)
    ...
    if userInput == 'y':
        for files in os.listdir(savePath):
            if files.endswith('.csv'):
                filePath = os.path.join(savePath, files)
                data=grab_data(filePath)
                plot_trials(windowSize=75, data=data, plot=True)
                plt.title('Synced Trial - Voltage vs Time')
                plt.xlabel('Voltage')
                plt.ylabel('Time')
                plt.grid(True)
                plt.tight_layout()
